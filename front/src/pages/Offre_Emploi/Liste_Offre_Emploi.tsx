...
import { useGetOffresQuery } from "../../store/api/OffreApp";
import { JobOffer } from '../../interfaces/mainInterfaces';
import '../Utilisateur/user.css';
...
const ListeOffreA = () => {
    ...
    return (
        <>
            <Breadcrumb pageName="Liste d'Offre d'Emploi" />
            <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                <div className="max-w-full overflow-x-auto">

                    <div className="d-flex flex-row align-items-center justify-content-between">

                        <Link to="/dashboard/Offre_Emploi/Form_Offre_A" className="btn btn-success mb-2 mt-3">Ajouter Offre Emploi</Link>

                        {/* <h2 className="text-center mb-4">Liste Offre Emploi</h2> */}
                    ...
                    <table className="w-full table-auto">
                        <thead>
                            <tr className="bg-gray-200 text-left dark:bg-meta-4">
                                <th className="min-w-[220px] py-4 px-4 font-medium text-black dark:text-white xl:pl-11">Titre Job</th>
                                <th className="min-w-[150px] py-4 px-4 font-medium text-black dark:text-white">Date Limite</th>
                            ...
                            {data && data.map((offre) => (
                                <tr key={offre.id}>
                                    ...
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">{offre.typeContrat}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p>{offre.statut}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <div className="flex items-center space-x-3.5">
                                            <Link to={`/dashboard/Offre_A/Edit_Offre/${offre.id}`} className="hover:text-primary">
                                                <svg className="fill-current" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M8.99981 14.8219C3.43106 14.8219 0.674805 9.50624 0.562305 9.28124C0.47793 9.11249 0.47793 8.88749 0.562305 8.71874C0.674805 8.49374 3.43106 3.20624 8.99981 3.20624C14.5686 3.20624 17.3248 8.49374 17.4373 8.71874C17.5217 8.88749 17.5217 9.11249 17.4373 9.28124C17.3248 9.50624 14.5686 14.8219 8.99981 14.8219ZM1.85605 8.99999C2.4748 10.0406 4.89356 13.5562 8.99981 13.5562C13.1061 13.5562 15.5248 10.0406 16.1436 8.99999C15.5248 7.95936 13.1061 4.44374 8.99981 4.44374C4.89356 4.44374 2.4748 7.95936 1.85605 8.99999Z" fill="currentColor" />
