...
import { useDeleteQuestionMutation, useGetQuestionsQuery } from "../../store/api/QuestionApp";
...
const ListeQuestion = () => {
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm("Êtes-vous sûr de vouloir supprimer cette évaluation ?")) {
            try {
                ...
                alert("Erreur lors de la suppression du Question.");
            }
        }
    };
    ...
    return (
        <>
            <Breadcrumb pageName="Liste de question" />
            <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                <div className="max-w-full overflow-x-auto">

                    <div className="d-flex flex-row align-items-center justify-content-between">

                        <Link to="/dashboard/Question/Form_Question" className="btn btn-success mb-2 mt-3">Ajouter Évaluation</Link>

                        {/* <h2 className="text-center mb-4">Liste des évaluations</h2> */}
                            ...
                            <tbody>
                                {data && data.map((Question) => (
                                    <tr key={Question.id}>
                                        ...
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                            <div className="flex Questions-center space-x-3.5">
                                                <Link to={`/dashboard/Question/Form_Question/${Question.id}`} className="hover:text-primary">
                                                ...
                                                <button onClick={() => handleDelete(Question.id)} className="hover:text-primary">
                                                    <svg className="fill-current" width="18" height="18" viewBox="0 0 18 18">
