...
//               <img src={th} alt="tlogo" style={{ width: '180px', height: '65px' }} />
...
import { useState } from "react";
import { Bars3Icon, XMarkIcon } from "@heroicons/react/24/solid";
import th from '../../../asset/images/user/th.png';  // Votre image
import { Link } from "react-router-dom";
import useMediaQuery from "../../../hooks/useMediaQuery";
...
const Navbar1 = ({ isTopOfPage }: any) => {
  ...
  return (
    <nav
      className="text-white text-lg bg-cover bg-center"
      style={{
        backgroundImage: `url(${th})`, // Utilisation de l'image comme fond
        backgroundPosition: 'center', // Centrer l'image
        backgroundSize: 'cover', // Couvrir toute la zone de la navbar
        backgroundRepeat: 'no-repeat', // Ne pas répéter l'image
      }}
    >
      <div
        className={`${navbarBackground} flex justify-between fixed top-0 z-30 w-full py-6`}
      > 
        <div className="mx-auto w-[91.67%]">
          <div className="flex justify-between w-full gap-16">
            <div className="flex items-center">
            ...
            {isAboveMediumScreens ? (
              ...
              >
                ...
                <div className="flex gap-8 text-sm" style={{ fontSize: '1.2rem' }}>
                  ...
                  <Link to="/web/offre" className="hover:text-green-500">Offre d'Emploi</Link>
                  <Link to="/web/postuler" className="hover:text-green-500">Postuler</Link>
                  <Link to="/web/login" className="hover:text-green-500">Suivre sa Candidature</Link>
                </div>
      ...
      {!isAboveMediumScreens && isMenuToggled && (
        <div className="fixed right-0 top-0 z-40 h-full w-[400px] bg-black drop-shadow-xl">
          ...
          >
            <Link to="/web" className="hover:text-green-500">
              Accueil
            </Link>
            <Link to="/web/offre" className="hover:text-green-500">
              Offre d'Emploi
            </Link>
            <Link to="/web/postuler" className="hover:text-green-500">
              Postuler
            </Link>
            <Link to="/web/login" className="hover:text-green-500">
              Suivre sa Candidature
            </Link>
            <div className="flex flex-row gap-4 justify-center">
              <Link
